// test/unit/devices.test.mjs —— 设备白名单模块单元测试。
...
test('isDeviceTrusted: returns false for empty/null/undefined token', async () => {
  // 动态导入以使用临时目录
  const { isDeviceTrusted } = await import(`../../src/auth/devices.js?t=${Date.now()}`);
...
test('isDeviceTrusted: returns true after approveDevice', async () => {
  const { approveDevice, isDeviceTrusted } = await import(`../../src/auth/devices.js?t=${Date.now()}`);
...
test('addPendingDevice: stores device with metadata', async () => {
  const { addPendingDevice, getPendingDevices } = await import(`../../src/auth/devices.js?t=${Date.now()}`);
...
test('removePendingDevice: removes existing pending device', async () => {
  const { addPendingDevice, removePendingDevice, getPendingDevices } = await import(`../../src/auth/devices.js?t=${Date.now()}`);
...
test('removePendingDevice: no-op for non-existent token', async () => {
  const { removePendingDevice, getPendingDevices } = await import(`../../src/auth/devices.js?t=${Date.now()}`);
  const before = getPendingDevices().length;
...
test('approveDevice: moves device from pending to trusted', async () => {
  const { addPendingDevice, approveDevice, isDeviceTrusted, getPendingDevices } = await import(`../../src/auth/devices.js?t=${Date.now()}`);
...
test('approveDevice: returns false for empty/null token', async () => {
  const { approveDevice } = await import(`../../src/auth/devices.js?t=${Date.now()}`);
...
test('denyDevice: removes device from both trusted and pending', async () => {
  const { addPendingDevice, denyDevice, isDeviceTrusted, getPendingDevices } = await import(`../../src/auth/devices.js?t=${Date.now()}`);
...
test('denyDevice: returns false for empty/null token', async () => {
  const { denyDevice } = await import(`../../src/auth/devices.js?t=${Date.now()}`);
...
test('getLatestPendingDevice: returns null when no pending devices', async () => {
  ...
  const { getLatestPendingDevice } = await import(`../../src/auth/devices.js?t=${Date.now()}`);
  assert.equal(getLatestPendingDevice(), null);
  cleanup();
});

test('getLatestPendingDevice: returns most recent device token', async () => {
  const { addPendingDevice, getLatestPendingDevice } = await import(`../../src/auth/devices.js?t=${Date.now()}`);
...
test('pending devices persist to file', async () => {
  const { addPendingDevice } = await import(`../../src/auth/devices.js?t=${Date.now()}`);
...
test('one devices module instance follows CODEX_DATA_DIR changes without crossing stores', async () => {
  ...
  const devices = await import(`../../src/auth/devices.js?t=${Date.now()}`);
  try {
...
test('device caches include the data file path when stores have identical mtimes', async () => {
  ...
  const devices = await import(`../../src/auth/devices.js?same-mtime=${Date.now()}`);
  try {
...
test('设备记录保留元数据，且能读回旧的扁平数组', async () => {
  const { addPendingDevice, approveDevice, getTrustedDevices, touchDevice } =
    await import(`../../src/auth/devices.js?meta=${Date.now()}`);
...
test('旧格式的扁平数组仍可读，缺失字段留空而不是伪造', async () => {
  ...
  const { getTrustedDevices, isDeviceTrusted } =
    await import(`../../src/auth/devices.js?legacy=${Date.now()}`);
...
test('批准设备时签发专属凭证，可据此认证且可单独撤销', async () => {
  const { addPendingDevice, approveDevice, issueDeviceSecret, verifyDeviceSecret, denyDevice } =
    await import(`../../src/auth/devices.js?secret=${Date.now()}`);
...
test('凭证不以明文落盘', async () => {
  const { addPendingDevice, approveDevice, issueDeviceSecret } =
    await import(`../../src/auth/devices.js?hash=${Date.now()}`);
...
test('denyDevice reloads before writing so a signature collision cannot lose an external change', async () => {
  const { denyDevice, isDeviceTrusted } = await import(`../../src/auth/devices.js?t=${Date.now()}`);
  const file = join(tempDir, 'trusted-devices.json');
...
test('issueDeviceSecret reloads before writing so a revoked device cannot be handed a fresh credential', async () => {
  const { issueDeviceSecret, isDeviceTrusted } = await import(`../../src/auth/devices.js?t=${Date.now()}`);
  const file = join(tempDir, 'trusted-devices.json');
...
test('verifyDeviceSecret 拒绝类型混淆：包着真凭证的数组不算凭证', async () => {
  const { approveDevice, issueDeviceSecret, verifyDeviceSecret } = await import(`../../src/auth/devices.js?t=${Date.now()}`);
...
test('denyDevice: 写盘失败时回滚并返回 false，不留下「内存已撤销、磁盘还信任」的假象', async () => {
  const { approveDevice, denyDevice, isDeviceTrusted } = await import(`../../src/auth/devices.js?t=${Date.now()}`);
...
test('touchDevice: persist 决定这次推进要不要落盘，默认落盘', async () => {
  const { approveDevice, touchDevice } = await import(`../../src/auth/devices.js?t=${Date.now()}`);
  const file = join(tempDir, 'trusted-devices.json');
...
test('getTrustedDeviceTokens 返回可信 token 的 Set，供广播过滤使用', async () => {
  const { approveDevice, getTrustedDeviceTokens } = await import(`../../src/auth/devices.js?t=${Date.now()}`);
  approveDevice('dev_a');
  approveDevice('dev_b');
...
test('设备记录归一化：空串和非正数一律变成 null，不是原样留下', async () => {
  const { getTrustedDevices } = await import(`../../src/auth/devices.js?t=${Date.now()}`);
  writeFileSync(join(tempDir, 'trusted-devices.json'), JSON.stringify([{
...
test('pending 文件里混进坏记录时，只丢掉坏的那条，其余待批设备保留', async () => {
  const { getPendingDevices } = await import(`../../src/auth/devices.js?t=${Date.now()}`);
