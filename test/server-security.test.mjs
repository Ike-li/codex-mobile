...
// 【这里曾有第二条：「认证失败的处理开销不随已见过的来源数增长」，2026-09-22 删除】
//
// 它想补上的是「上界带来的后果」：光有上界、扫描仍是 O(n) 的话照样能把网关拖死。
// 判据是两个都落在上界之上的批次的耗时比，要求 `farPast / atCap < 3`。
//
// **删除理由不是它偶发抖动，是它的判据方向反了。** dev 上复刻两种实现各跑三轮实测：
//
// | 实现 | 比值 | 该测试 |
// |---|---|---|
// | 正常（trim 提前 break） | 3.6 ~ 4.1× | **红** |
// | 退化（trim 走完全表） | 1.0 ~ 1.1× | **绿** |
//
// 退化实现每次恒定扫全表，两端都慢，比值反而趋近 1；正常实现 atCap 极小（~3ms）、
// farPast 略大（~12ms，V8 的 Map 在大量 delete 后迭代变慢），比值天然超过 3。
// 本次移植前在 macOS/node 25 上独立复核过：当前这份（正确的）实现三轮里红一轮，
// 失败那轮比值 3.2×——在比 CI runner 快得多的机器上尚且如此。
//
// 没有改小的修法：放宽阈值会让两种实现都通过（假红变假绿）；换零计时的行为判据也不行，
// 唯一能区分 break 与不 break 的是「过期条目有没有被顺带清掉」，而 recordAuthFailure
// 对过期窗口会就地重建、`count` 两种情况都回到 1，表长与 has() 又都没导出；
// 就算导出，server.js 里 trimAuthFailureWindows 的注释明写着它**只保证上界、不保证过期回收**，
// 钉住「过期条目被清掉」等于把实现细节当契约。
//
// 而它想守的性质本就被上面那条蕴含：表有上界 ⇒ 每次 trim 最多扫 AUTH_WINDOW_MAX_ENTRIES 条，
// 与「见过的来源数」无关。上界由上面那条用零计时的行为判据守着，够了。
