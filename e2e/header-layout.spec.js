...
test.describe('顶部导航紧凑度', () => {
  test('标题栏压薄且不折行', async ({ page }) => {
    ...
    await expect(page.locator('#header-home')).toHaveCount(0);
    ...
    await expect(page.locator('#conn-rtt')).toBeHidden();
  });
  ...
  test('工作区胶囊打开 sheet，首页清空当前会话视图', async ({ page }) => {
    ...
    await expect(page.locator('#header-thread')).toBeVisible();
    await expect(page.locator('#header-thread')).toHaveText('Mock thread');

    await page.locator('#header-new').click();
    await expect(page.locator('#empty-state')).toBeVisible();
    await expect(page.locator('#header-thread')).toBeHidden();
  ...
  test('工具控制栏收进抽屉,顶部不再有常驻工具行', async ({ page }) => {
    ...
    // 顶部不再有工具入口行。协议按钮已从抽屉拿掉：压缩/回退进会话长按，
    // 文件走工作区胶囊，账号类走设置与状态。
    await expect(page.locator('#native-controls-toggle'), '顶部工具入口应已移除').toHaveCount(0);
    await expect(page.locator('#native-controls')).toHaveCount(0);
    await expect(page.locator('#drawer-tools')).toHaveCount(0);

    await page.locator('#menu-btn').click();
    await expect(page.locator('#drawer')).toHaveClass(/open/);
    await expect(page.locator('#btn-general-settings'), '设置与状态仍是抽屉里的主机入口').toBeVisible();
