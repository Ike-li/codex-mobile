...
**行距判据是绝对增量，不是比值**（2026-09-13 改，抄 ChatGPT 的 `--markdown-line-height: calc(font-size + 6px)`）。倍数行高在字号变化时行距等比放大，大字号被推散、小字号又挤；绝对增量让两档字号保持同一条呼吸节奏。

改这条时踩到一个反直觉的地方：**比值会随字号反向变化**。6px 在小字号里占比更大，于是用户气泡（16px→22px，比值 1.375）反而高于助手正文（17px→23px，比值 1.353）。原来那条「紧凑气泡的行高比应低于长文阅读态」在新公式下**恒假**——它不是回归，是判据本身过期了。换成守 `lineHeight - fontSize ≈ 6px`：有人改回倍数行高，两档立刻红，而这正是该守的东西。
...
**整圈边框只做轮廓，类型和状态都编码在左边那一条。** 此前命令卡用整圈变绿表示成功，而成功是常态、不该被高亮，结果是滚动时满屏绿框、真正要注意的反而不突出。现在成功刻意不改色（`exit` code 已经用 `.tool-ok` 标过一次），只有 `data-ok="false"` 把左边框变红——需要在滚动中一眼找到的是失败。

2026-09-14 去绿后，`.tool-ok` 从绿字变成 `--text-muted` 的灰字，上面这条逻辑反而更彻底了：成功现在连颜色都不占，只有失败是红的。顺带修掉一个既存缺陷——它原先写死 `#87e0a2`（深色档的薄荷绿）却放在通用规则里，浅色模式下 on `#ffffff` 只有 1.8:1，那次「已经标过一次」在浅色下其实基本看不见。
