...
基线在 `.coverage-baseline.json`（当前 **90.5 / 85.96 / 92.89 / 90.5**），门禁是**相对基线跌幅 ≤2pp**，由 `scripts/gates/check-coverage-delta.js` 执行。排除项在 [.c8rc.json](../.c8rc.json)——`public/vendor/**` 是第三方压缩代码，不该计进我们的覆盖率。这四个数字与基线文件的一致性由 `test/invariants/acceptance-doc.test.mjs` 盯着：只改文件不改文档的话，这一段会继续教人拿一个错的参照去判断自己的改动。
...
**不要用碎测试去凑数字。** 分支覆盖已回到 85.96%（5073/5901），离历史高点 87.88% 还差 1.92pp、约 113 条分支。缺口集中在两个文件：`server.js` 未覆盖 270 条、`src/agent/agent-appserver.js` 未覆盖 95 条——**这两个就超过缺口总量的三倍**。所以补它是一项针对两个最大模块错误路径的独立工作，不是清理。

**2026-09-15 刷新基线的记录。** 上一版基线是 91.27 / 80.12 / 95.09 / 91.27，其中 functions 一项对真实值高了 2.24pp，`check-coverage-delta` 因此在 `dev` 上**一直是红的**，卡住整条 `test:ci`。归因不是保护变弱了，是**分母里进来了一层本来就不打算单测的代码**：B0–B10 新增的 `scripts/{config,qr,setup,doctor}.js` 是 CLI 宿主层，判定逻辑按三层设计留在 `src/ops/` 里。按目录拆开看得很清楚——

| 分组 | 函数覆盖 | 说明 |
|---|---|---|
| `src/` | 96.80% (454/469) | 判定与探测都在这里，比旧基线的 95.09 还高 |
| `server.js` | 94.74% (90/95) | |
| `public/js/` | 88.67% (180/203) | |
| `scripts/` | 83.33% (100/120) | 维护者 CLI，宿主层刻意不单测 |

去掉 `scripts/` 后全仓函数覆盖是 94.39%。同一轮里 branches 反而从 80.12 涨到 85.96（+5.84pp），因为那些 CLI 的判定层被测得很厚——**functions 跌和 branches 涨是同一件事的两面**，只看一个数字会得出相反的结论。
...
| 案例 | 场景 | 代码入口 | 主要证据 |
|---|---|---|---|
| 案例 1 | 创建任务 + 流式输出 + ACK/outbox + provisional orphan/fresh-id 恢复 + gap 后恢复会话 | `server.js`、`src/sessions/message-receipt-ledger.js`、`public/js/compose/message-request.js`、`public/js/outbox/message-outbox.js`、`public/js/outbox/{indexeddb-outbox,outbox-recovery,recovery-state}.js` | receipt/dedup 集成测试、outbox 与 recovery 单测、关键流程和 outbox recovery E2E |
...
| 案例 7 | 多工作目录 + 实例切换 + 双设备/双 thread 零串流 + 共享单进程 | `src/agent/app-server-host.js`、`src/sessions/thread-registry.js`、`src/agent/agent-appserver.js`、`public/js/ui/view-routing.js` | shared-host spawn/initialize、stale target、route/workdir、多实例 E2E |
